...
export default function SignUp() {
    ...
    const onSuccess =(res)=>{
        enregistrer(res.profileObj.email,res.profileObj.name,res.profileObj.name);
    };

    const onFailure =(res)=>{
        setErreur({etat:true,message:"Erreur de connexion"})
    }

    /*Fonctions de gestion de connexion avec google login **/

    const reponseFacebook =(reponse)=>{    
        enregistrer(reponse.email,reponse.name,reponse.name);
    }
    
    

    const enregistrer = (sendEmail,sendPassword,sendName)=>{
        ...
        <div className="sign center">
            <div className="sign flexV">
                ...
                <div className="formulaire center flexV">
                    ...
                    <input className="input" type="text" placeholder="Nom et Prenom(s)" onChange={(e)=>{setName(e.target.value)}}/>
                    ...
                    <div className="button center" onClick={(e)=>{enregistrer(email,password,name)}}>S'enregistrer</div>  
